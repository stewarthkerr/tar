...
tar <- merge(tar,tar_mrn,by='studyid',suffixes=c('.DROP','')) %>% select(-contains('.DROP')) %>%
      filter(tka=='0' | is.na(tka)) #Only want patients that haven't had a tka
      
tar<-mutate(tar,age_at_surgery=((year(mdy(surgery_date)))-(year(mdy(dob))))) #calculate age at time of surgery

for (i in base$iteration_num){
  work <- filter(base,iteration_num==i)
  work_pd <- ifelse(i==1,c('1','2','3'),work$primary_diagnosis)
  #work_age <- work$age_at_surgery
  dn <- paste('FILTERED',work$mrn,work$sx_side_v2,sep='_')
  an <- paste('ALL',work$mrn,work$sx_side_v2,sep='_')
  
  work_tar <- filter(tar,attending==work$attending,
                     prosthesis==work$prosthesis,
                     primary_diagnosis==work$primary_diagnosis)
                     #year(mdy(surgery_date))==year(mdy(work$surgery_date)),
                     #((age_at_surgery>=(work_age-5)&age_at_surgery<=(work_age+5)) | (age_at_surgery<=60 & work_age<=60))) %>%
  
  #Need more patients to match for this one
  if(i==1){
    work_tar <- filter(tar,attending==0,
                       prosthesis==0,
                       primary_diagnosis==1 | primary_diagnosis==2 | primary_diagnosis == 3)
          }
  
  if(i>1 & i<6){
    work_tar <- filter(tar,attending==0,
                       prosthesis==1,
                       primary_diagnosis==1 | primary_diagnosis==2)
                }
  
  if(i==11 | i==12){
    work_tar <- filter(tar,attending==1,
                       prosthesis==2,
                       primary_diagnosis==3)
                    }
